...
class ReflexAsyncManager:
    """Manages asynchronous action chunk execution for Reflex inference.
    
    This class implements the core Reflex async inference strategy:
...
def run_loop(
    ...
    # Initialize async manager for Reflex inference
    ...
    async_manager = ReflexAsyncManager(
        policy=policy,
        robot=robot,
...
def run(cfg: RunConfig):
    """Main entry point for Reflex robot inference.
    
    Loads a pretrained policy and runs it on a connected robot using
    Reflex's async inference strategy for real-time control.
    ...
    if cfg.display_data:
        init_rerun(session_name="reflex_run")
    ...
    log_say("Starting Reflex run", cfg.play_sounds, blocking=True)
    ...
    try:
        ...
        log_say("Stopping Reflex run", cfg.play_sounds, blocking=True)
        robot.disconnect()
