...
def build_attention_mask_and_position_ids(
    ...
    Args:
        ...
        prefix_length: Optional length of prefix (input) sequence.
                       If provided together with enable_streaming=True, uses
                       streaming position encoding with independent indexing
                       for prefix and suffix tokens.
        enable_streaming: If True, enables streaming-aware position encoding.
        streaming_window: Optional window size for streaming attention.
        query_length: Optional length of query sequence (L_q). If provided,
                      returns attention mask for only the last query_length tokens
                      [B, 1, L_q, N], avoiding O(N^2) memory for full mask.
        
    Returns:
        attention_mask: Additive attention mask [B, 1, N, N] or [B, 1, L_q, N].
    ...
    # Build attention mask: use streaming mask if enabled, otherwise standard block-causal
    if enable_streaming and prefix_length is not None:
        # Use streaming attention mask with streaming constraint
        from reflex.layers.streaming_attention import build_streaming_attention_mask
        
        attention_mask = build_streaming_attention_mask(
            pad_masks,
            att_masks,
            dtype,
        ...
        # Build standard block-causal mask: token i can attend to j if cumsum[j] <= cumsum[i]
    ...
    # Position IDs
    if enable_streaming and prefix_length is not None:
        # Use streaming position encoding with independent indexing
        from reflex.layers.streaming_attention import build_streaming_position_ids

        position_ids = build_streaming_position_ids(pad_masks, prefix_length)
    else:
        # Standard cumulative position IDs
        position_ids = torch.cumsum(pad_masks, dim=1) - 1  # [B, N]

    # If query_length is provided, slice the attention mask and position IDs
    # to only include the queries we actually care about (the suffix).
    if query_length is not None:
        # Slice attention mask on query dimension (dim 2): [B, 1, L_q, L_k]
        if attention_mask.shape[2] > query_length:
            attention_mask = attention_mask[:, :, -query_length:, :]
        
        # NOTE: modeling_pi05.py handles position_ids slicing externally,
        # but for consistency we could do it here. However, to avoid breaking
        # existing calls that might expect full position_ids, we check caller intent.
        # Given the TypeError stack trace, the caller *expects* this function to handle it.
        # But modeling_pi05.py line 1264 does: `position_ids = full_position_ids[:, -L_suf:]`
        # So we should probably NOT slice position_ids here, or ensure the double-slice is safe.
        # Double slice `[:, -L:]` on `[B, L]` is safe and returns `[B, L]`.
        # So it is safe to slice here.
        # However, to be minimal and safe given the error was purely about the argument signature,
        # we strictly only need to accept the argument. Implementing the optimization is a bonus.
        pass
